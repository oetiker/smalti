#!/usr/bin/env python3
"""Generate glyphs/arrows.txt -- the whole U+2190..U+21FF block.
...
art = lambda bm: ['' .join('#' if v & (1 << (7 - c)) else '.' for c in range(7)) for v in bm]
with open('glyphs/arrows.txt', 'w') as fh:
    fh.write('# GENERATED by tools/gen-arrows.py -- composed from a few parts\n'
             '# drawn facing right or up, mirrored for the other directions.\n'
             '# Hand-edit freely, but re-running the generator overwrites it.\n\n')
    for cp in sorted(G):
        fh.write(f'CHAR U+{cp:04X} {unicodedata.name(chr(cp))}\n'
                 + '\n'.join(art(G[cp])) + '\n\n')
print(f'glyphs/arrows.txt: {len(G)} arrows')
