...
* **`ª` U+00AA and `º` U+00BA are the only two holes left in Latin-1.**
  Everything else in U+0080..U+00FF that is a character at all is drawn.
  Found by the specimen site's block table, which is what that table is for.
